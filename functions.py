...
def words_nation(corpus_directory, president_names):
    tf_idf_matrix = calculate_tf_idf(corpus_directory)
    ...
    if word_indices:
        president_word_count = defaultdict(int)
        president_without_duplicates = set()

        for i, doc in enumerate(tf_idf_matrix):
            president_name = list_of_files(corpus_directory, ".txt")[i].split("_")[1].capitalize()
            president_without_duplicates.add(president_name)
        ...
        if president_word_count:
            ...
            return {
                "Presidents": list(president_without_duplicates),
